...
const flameShader = {
  ...
  fragmentShader: glslify( /*glsl*/ `
    ...
    void main () {
      vec2 grid = floor(localPosition.xy * grid) / grid;
  ...
  vertexShader: glslify(/* glsl */`
    ...
    void main () {
      ...
      worldPosition = modelViewMatrix * vec4(position.xyz, 1.0);
...
const sketch = ({ context, width, height }) => {
  ...
  const gridSize = random.rangeFloor(6,14);
  const voxelSize = random.range(0.01,0.25);
  const hueRandomness = random.range(1,30);
  ...
  // add flames
  const flames = _.range(-0.5, 0.5, voxelSize).map( (val, index) => {
    const flame = new THREE.Mesh(
      new THREE.BoxBufferGeometry(1, 1, voxelSize*0.99),
  ...
  return {
    ...
    render ({time}) {
      ...
      var angle = startAngle + time * 0.2;

      camera.position.set(Math.sin(angle)*CAMERA_RADIUS, 0.9, Math.cos(angle)*CAMERA_RADIUS);
      camera.lookAt(new THREE.Vector3(0,0.15,0));

      flames.forEach( (flame, index) => {
        setShaderUniforms(flame.material, { time: time * 0.5, hue : time/20.0 + index / hueRandomness })
      })
